<template>
  <div class="am-picker"
    ...
    <div class="am-box"
      ...
      <div class="am-ul">
        <div class="am-item"
             v-for="(item,m) in option"
             :key="m"
             :class="m==0?'':'other'"
             @touchmove.prevent=touchmove($event,m)
             @touchstart.prevent=touchstart($event,m)
             @touchend.prevent=touchend($event,m)>
          <ul :style="'transform: translate3d(0px, '+topStyle[m]+'px, 0px)'"
              :class=endClass[m]
              style="list-style:none;">
            <li v-for="key in item"
                :key="key"
                :class="key==value[m]?'active':''">{{key}}</li>
          </ul>
        </div>
